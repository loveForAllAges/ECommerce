$(document).ready(function(){
    ...
    function updateCart(data) {
        ...
        data.goods.forEach(item => {
            var price_per_once = `<p class="text-sm text-gray-500">1 шт. / ${ item.product.price } ₽</p>`
                    ...
                    <div class="flex flex-col flex-1 space-y-1 justify-between">
                        <a href="${ item.product.url }" class="flex space-x-4">
                            ...
                            <div class="flex flex-col">
                                ...
                                <div class="mt-1 text-sm text-gray-500">${ item.size } размер</div>
                            </div>
                        </a>
                        <div class="ml-4">
                            <span class="isolate inline-flex rounded-md border border-gray-200">
                                <button type="button" data-product="${ item.product.id }" data-action="minus" data-size="${ item.size }" class="cartBtn relative inline-flex items-center justify-center p-1 text-gray-400 hover:text-gray-500 duration-150">
                                ...
                                <button type="button" data-product="${ item.product.id }" data-action="plus" data-size="${ item.size }" class="cartBtn relative inline-flex items-center justify-center p-1 text-gray-400 hover:text-gray-500 duration-150">
                                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
